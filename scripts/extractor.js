...
async function main() {
  ...
  try {
    ...
    console.log(
      `Filtrage terminé. ${filteredAds.length} annonces conservées (avec une référence).`
    );
  ...
  if (adsDataToInsert.length > 0) {
    ...
    for (const ad of adsDataToInsert) {
      try {
        ...
        console.error(
          `Erreur lors de l'insertion de l'annonce avec référence ${ad.reference}:`,
          dbError
        );
      }
    }
    console.log(
      `Insertion terminée. ${insertedCount} nouvelle(s) annonce(s) ajoutée(s) à la base de données.`
    );
  }
